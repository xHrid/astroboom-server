...
io.on("connection", socket=> {
    ...
    socket.on(emitName.handshake, (pName, lobbyCode, mode, pEmail)=>{
        ...
        if (activePlayers.hasOwnProperty(pEmail)) {
            ...
            console.log("New Connection for lobby: ", lobbyCode);
            if (lobbyCode == "") {
                ...
                if (mode == 'j') openRooms.forEach((lc, i) => {
                    if (lc == lobbyCode) {
                        socket.join(lobbyCode);
                        activePlayers[pEmail]= lobbyCode;
                        rooms[lc].push({socket: socket, name: pName, email:pEmail, p1Data, buildCount: 0});
                        ...
                        rooms[lc][0].socket.emit(p2Details, {p2: pName, p1Data, p2Data: rooms[lobbyCode][0].p1Data,  yourTurn: p1Turn, position: {
                            i: Math.floor(p1Position/2),
                            j: p1Position%2
                        }})
                        socket.emit(p2Details, {p2: rooms[lc][0].name, p1Data: rooms[lobbyCode][0].p1Data, p2Data: p1Data, yourTurn: !p1Turn, position: {
                            i: Math.floor(p2Position/2),
                            j: p2Position%2 
                        }})
